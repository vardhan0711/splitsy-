...
export const getAllContacts = query({
  handler: async (ctx) => {
    ...
    /* ── fetch user docs ───────────────────────────────────────────────── */
    let contactUsers = await Promise.all( // Changed to 'let' because we'll reassign
    ...
    /* sort alphabetically */
    // Now 'a' and 'b' are guaranteed to be objects with a 'name' property (or at least not null/undefined)
    contactUsers.sort((a, b) => (a.name || "").localeCompare(b.name || "")); // Added defensive check for 'name' too, just in case it's missing or null in the DB
    userGroups.sort((a, b) => a.name.localeCompare(b.name));

    return { users: contactUsers, groups: userGroups }; // No need to filter(Boolean) here again
  },
});
