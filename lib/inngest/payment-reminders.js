    ...
    const results = await step.run("send‑emails", async () => {
      return Promise.all(
        users.map(async (u) => {
            ...
            .map(
              (d) => `
                <tr>
                  ...
                  <td style="padding:4px 8px;">₹${d.amount.toLocaleString('en-IN', { style: 'currency', currency: 'INR', minimumFractionDigits: 2 }).replace(/^\D+/,'')}</td>
                </tr>
              `
            )
            .join("");
          ...
          const html = `
            <h2>Splitsy – Payment Reminder</h2>
          ...
          try {
            await convex.action(api.email.sendEmail, {
              to: u.email,
              subject: "You have pending payments on Splitsy",
              html,
